...
이왕 자동 미분기능을 알았으니, 이 기능을 이용하여 식 \@ref(eq:grad_decent)의 경사하강법으로 함수값을 최소로 만드는 $\mu$ 값을 찾아보도록 하자.
...
`with_no_grad({})`의 경우, 안에 들어있는 코드들 중 미분 추적 태그가 붙어있는 경우라도 미분 추척 기능을 활성화하지 않는다. `mu$grad$zero_()` 부분은 미분값을 초기화 해주는 부분이라고 이해하면 좋다. 그렇지 않을 경우 이전의 값이 남아있어서 계속 누적이 되게 된다.

### 시각화

```{r}
mu_points <- tibble(x = result, 
                    y = map_dbl(result, ~as.numeric(f(mu = .x))))
p +
...
이것으로 자동 미분 기능에 대하여 알아보았다. 이 기능을 활용하면 훨씬 복잡한 구조의 함수(예를 들어 딥러닝에서의 신경망 같은)에 대한 미분값 역시도 쉽게 구할 수 있다.
