...
토치를 사용해서 신경망을 정의할 때 사용하는 함수가 있다. 바로 `nn_module()`이라는 함수인데, `torch`에서 신경망을 정의할 때, 이 함수를 사용해서 "클래스"를 만들어 정의한다! 왜 우리가 챕터 \@ref(r6)에서 그리 R6관련 클래스 내용을 오지게 공부했었는지에 대한 답을 바로 이 챕터에서 찾을 수 있을 것이다.
